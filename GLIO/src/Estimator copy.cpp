...
private:
    ...
    ros::Publisher pub_odom_rtk_ini, pub_path_rtk_ini; // pub rtklib traj  revise rtk

    ros::Publisher pub_gnss_lla;  //revise
    ros::Publisher pub_tc_enu_path, pub_lc_enu_path;  //revise
    ...
    ros::Publisher span_BP_pub; //publish gt data by txt reading  //revise

    ros::Publisher pub_sub_gl_map; // publish sub map

    nav_msgs::Path rtk_ini_enu_path, tc_enu_path, spp_path;  //revise 用于存储路径信息
    nav_msgs::Odometry odom_mapping; // odom from tc fusion and lc fusion
    nav_msgs::Odometry odom_init_kf; //
    std_msgs::Float32MultiArray rtk_infos;   //存储RTK信息 revise
    ...
    deque<pcl::PointCloud<PointType>::Ptr> recent_surf_keyframes;  //最近的关键帧点云队列
    int latest_frame_idx;

    pcl::KdTreeFLANN<PointType>::Ptr kd_tree_surf_local_map;  //最近邻搜索的KD树
    pcl::KdTreeFLANN<PointXYZI>::Ptr kd_tree_his_key_poses;

    vector<int> pt_search_idx;  //用于存储最近邻点的索引。
    vector<float> pt_search_sq_dists;  //用于存储最近邻点的距离。
    ...
    PointPoseInfo last_GNSS_add_pos;  //最后添加的GNSS位置

    // Loop closure detection related
    bool loop_to_close;  //闭环检测标志
    int closest_his_idx; //最近的历史关键帧索引
    int latest_frame_idx_loop; //最新的关键帧索引
    bool loop_closed;  //闭环是否已关闭
    ...
    double reflect_thres = 15;  //revise in the yaml
    ...
    vector<double> keyframe_time; //关键帧时间
    ...
    /* imu related */
    vector<sensor_msgs::ImuConstPtr> imu_buf;  //IMU数据缓存
    nav_msgs::Odometry::ConstPtr odom_cur;   //当前里程计信息
    vector<nav_msgs::Odometry::ConstPtr> each_odom_buf;  //每个里程计数据缓存
    double time_last_imu;  //上一次的IMU时间
    double cur_time_imu;  //当前的IMU时间
    bool first_imu;   //是否是第一个IMU数据。
    vector<Preintegration*> pre_integrations; //预积分对象
    ...
    vector<vector<double>> rcv_dt;  //revise
    ...
    Eigen::Matrix4d T_gnss_imu = Eigen::Matrix4d::Identity();  //GNSS到IMU的外参

    /* gnss measurements from reference station 
    * the time frame should at least 1 second (<1Hz)
    */
    double station_x_, station_y_, station_z_;  //参考站坐标

    std::map<int, nlosExclusion::GNSS_Raw_Array> station_gnss_raw_map; //参考站GNSS原始数据。
    std::map<int, nlosExclusion::GNSS_Raw_Array> gnss_raw_map;  //接收机gnss原始数据
    GNSS_Tools m_GNSS_Tools;  //GNSS工具对象
    ros::Subscriber sub_rtkpos_odometry_float, sub_rtklib_gnss_meas, sub_rtklib_ref_gnss_meas; //revise  GNSS相关订阅
    ...
    std::mutex m_buf;  //互斥锁
    double latest_gnss_time = 0;  //最近的GNSS时间
    double last_gnss_time = 0;  //上一次的GNSS时间

    Eigen::Vector3d anc_ecef, lever_arm_t; //锚点坐标和杆臂
    Eigen::Matrix3d R_ecef_enu;  // ECEF到ENU的旋转矩阵
    double yaw_enu_local;  //ENU到本地的偏航角。
    double initial_rpy_[3] = {0};  //初始的RPY（滚转、俯仰、偏航）角。
    Eigen::Quaterniond initial_Quat = Eigen::Quaterniond::Identity();  //初始四元数。
    double timeshift_IMUtoGNSS;  //IMU到GNSS的时间偏移。

    double para_anc_ecef[3]; // anchor point in ECEF frame  ECEF坐标系下的锚点。
    double para_yaw_enu_local[1]; // yaw offset from local to enu  本地到ENU的偏航角
    double para_rcv_ddt[EPOCH_SIZE]; // receiver clock bias drift  //接收器时钟偏移
    double gl_para_rcv_ddt[EPOCH_SIZE]; // receiver clock bias drift  // 接收器时钟偏移。

    Eigen::Matrix3d R_enu_local = Eigen::Matrix3d::Identity();  // ENU到本地的旋转矩阵
    Eigen::Vector3d ecef_pos, enu_pos, enu_ypr;  //ECEF和ENU坐标、方位角。

    bool GTinLocal, LCinLocal, RTKinLocal; // visualization  可视化标志

    deque<nav_msgs::Odometry> GNSSQueue;  //GNSS数据队列

    /* DD measurements related */
    std::map<int, int> epoch_time_idx; // epoch time and correspondence epoch index   GNSS历元时间索引
    int epoch_idx = 0; // epoch index updated by newly coming signal  历元索引
    std::vector<std::vector<double>> gt_time_poses; //vector (time, pose)  GT时间和位姿

    Eigen::Vector3d m_pose_enu; // pose in enu from initialization  ENU坐标系下的位姿
    std::string result_path, tc_sw_result_path, lc_result_path, batch_result_path;  //结果路径
    std::string result_path_evo, batch_result_path_evo, lc_result_path_evo;

    /* subscribe the ground truth */
    ros::Subscriber span_BP_sub;  //地面真值订阅器
    GNSS_Tools gnss_tools_1;   //GNSS工具对象
    Eigen::MatrixXd original1;  //referene point in llh   参考点
    bool flag=0; // get the reference point or not? double original_heading = 0;
    nav_msgs::Odometry spanOdom;  //地面真值里程计
    ros::Publisher pub_path_gt;  //地面真值路径发布器
    nav_msgs::Path path_gt;  //地面真值路径
    
    //first sliding window optimazition
    bool first_opt;  //是否是第一次优化

    // for marginalization
    MarginalizationInfo *last_marginalization_info;  //边缘化信息
    vector<double *> last_marginalization_parameter_blocks;  //边缘化参数块
    ...
    bool marg = true;  //边缘化标志。

    vector<int> imu_idx_in_kf;  //IMU索引。
    double time_last_loop = 0;  //上次闭环检测的时间。

    string imu_topic;  // IMU话题名称

    double surf_dist_thres;  //平面距离阈值
    double kd_max_radius;   //KD树最大半径
    bool save_pcd = false;  //是否保存PCD文件


    double lidar_const = 0;  //激光雷达常数
    int mapping_interval = 1;  //地图更新间隔
    double lc_time_thres = 30.0; //闭环时间阈值
    int start_idx = 0;  //起始索引

    string frame_id = "GLIO";
    string data_set;  //数据集名称 revise
...
public:
    Estimator(): nh("~") {
        ...
        // 如果数据集是"HK"，则订阅Novatel INS数据 //revise
        if (data_set == "HK") {
    ...
    void initializeParameters() {
        ...
        //初始化 ISAM2 参数
        //用于增量式平滑和映射（Incremental Smoothing and Mapping）。设置重线性化阈值和跳跃步数。
        ...
        //从 YAML 文件中加载参数,将某些参数存储在 rtk_infos 中。
...
//        res_lc_evo_output.close();
        ...
        //设置滑动窗口内的状态变量，包括旋转矩阵、位置、速度、加速度偏置和陀螺仪偏置。
        ...
        //初始化体素滤波器的叶大小，用于点云降采样。设置初始位姿信息的帧 ID。
        ...
        //设置 GTSAM 中使用的噪声模型，用于优化过程中的先验噪声和里程计噪声。
        ...
        /* extrinsic parameters */
        //设置外参（如 IMU 和激光雷达之间的旋转和平移），并初始化 GNSS 相关的变换矩阵。
    ...
    * @return void
    @ 
    */
    //处理来自 SPAN-CPT 的 GNSS 数据，并将其转换为 ENU 坐标系，同时发布相关的 ROS 话题。
    /******
    函数目的:
    1.处理 GNSS 数据，将其从经纬度（LLA）转换为东北天（ENU）坐标系。
    2.使用转换后的 ENU 数据创建并发布里程计消息（odometry message）和路径（path message）。
    *****/
    //revise
    void span_bp_callback(const novatel_msgs::INSPVAXConstPtr& fix_msg)
    {
        if(GTinLocal)
        {
            //1.GNSS 时间处理：从消息中提取 GPS 时间并转换为秒。
            ...
            //2.LLA 转 ECEF：经纬度高度（LLA）转换为地心地固坐标系（ECEF）。
            ...
            //3.初始值设置：在第一次接收到数据时，保存初始的 LLA 值，用于后续的 ENU 转换。
    ...
    /******
    rtklib_ref_gnss_meas_callback 函数用于处理来自参考站的 GNSS 数据，计算参考站位置，并将数据存储在一个映射中。以下是详细步骤：
    1.处理参考站的 GNSS 数据，计算其加权最小二乘法位置解。
    2.将 GNSS 数据存储在一个映射中，以便后续使用。
    *******/
    //revise
    void rtklib_ref_gnss_meas_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr &gnss_msg)
    {
        ...
        //使用加权最小二乘法计算参考站的 ECEF（地心地固坐标系）位置。
        ...
        //将 ECEF 坐标转换为 ENU 坐标系，以便更好地表示参考站的位置。
        ...
        //计算从参考站到真值位置的距离，并将数据存储在 station_gnss_raw_map 中。如果距离小于一定阈值（此处为 30 米），则认为数据有效，将其存储。
    ...
    /*****
    该函数将 GNSS 原始数据数组转换为以 PRN（伪随机噪声码）为键的地图格式。每个 GNSS 数据包含一个 PRN 索引，用于标识特定的卫星。
    1.将 nlosExclusion::GNSS_Raw_Array 数据转换为 std::map<int, nlosExclusion::GNSS_Raw> 格式。
    2.使 GNSS 数据能够以 PRN 为键进行高效访问。
    *****/
    bool gnssRawArray2map(nlosExclusion::GNSS_Raw_Array gnss_data, std::map<int, nlosExclusion::GNSS_Raw> &epochGnssMap)
    {
        ...
        {
            //使用 PRN 作为键，将每个 GNSS 数据项插入到 epochGnssMap 中。
    ...
    */
    //该函数在接收机和参考站的 GNSS 数据中查找一个共同的主卫星 PRN。
    //主卫星用于差分 GNSS 计算，它应该具有最高的高度角，并且来自同一个卫星系统（如 GPS、北斗等）。
    //revise
    bool findMasterPrn(std::map<int, nlosExclusion::GNSS_Raw> epochRcvGNSSMap, std::map<int, nlosExclusion::GNSS_Raw> epochStGNSSMap, int &masterPrn, int targetPrn)
    {
        ...
        //3.遍历接收机 GNSS 数据：检查是否找到主卫星,只有在找到的卫星系统与目标 PRN 的卫星系统相同时,并且在参考站中也存在该卫星时，才会将其设置为主卫星。
    ...
    //revise
    //该函数计算 ObsPtr 中卫星观测数据的方差（即 sigma square）。方差计算是基于卫星的高度角和信噪比（SNR）
    bool calVar(ObsPtr &obs)
    {
        double var = 0;

        /* variance for r_i_sat */
        //调用 eleSRNVarCal 函数，根据 r_i_sat 的高度角和信噪比计算方差，并赋值给 r_i_sat.var。
     ...
     * @return weight_matrix
     @ 
    */
   //该函数计算卫星高度角（Elevation）和信噪比（SNR）的方差（Sigma Square），并返回其值。
   //这个函数用于评估GNSS测量数据的质量，特别是在不同高度角和信噪比情况下的不确定性。
    double eleSRNVarCal(double ele, double snr)
    {
        int model = 0; // 0: ele&&SNR model. 1: RTKLIB ele model. 2: DLR Ele model  model：定义方差计算模型。
        Eigen::Matrix<double,4,1> parameters;   //arameters：存储SNR模型的参数。
        ...
        //根据高度角和信噪比计算方差的不同部分（q_R_1, q_R_2, q_R_3）
        //最终的方差 q_R 是这几个部分的乘积。
        double snr_R = snr;
        // snr_R = 40;
        double elR = ele;
    ...
    /* subscribe the odometry from RTKLIB in ECEF*/ //revise
    //该函数处理从RTKLIB接收的ECEF格式的里程计数据，将其转换为ENU格式并发布。
    void rtklibOdomHandler(const nav_msgs::Odometry::ConstPtr& odomIn) {
        if(RTKinLocal)
        {
            ...
            Eigen::Matrix<double, 3,1> ENU_ref;
            ENU_ref << 0, 0, 0;
            ENU_ref = m_GNSS_Tools.ecef2llh(anc_ecef); //将ECEF格式的基准点转换为LLH
            Eigen::Matrix<double, 3, 1> ENU;
            Eigen::Matrix<double, 3, 1> ECEF;
            ECEF<<odomIn->pose.pose.position.x, odomIn->pose.pose.position.y, odomIn->pose.pose.position.z; //revise
            ENU = m_GNSS_Tools.ecef2enu(ENU_ref, ECEF);  //当前的ECEF坐标转换为ENU坐标
    ...
    //从GNSS接收机接收测量数据，并找到对应的测站数据，进行双差处理，计算协方差，并确保数据的顺序正确，然后将其添加到GNSS测量向量中。这些数据最终可以用于GNSS定位算法中。
    void rtklib_gnss_meas_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr &meas_msg)
    {
        ...
        */
        ...
        for (size_t i = 0; i < length; i++)
        {
            ...
            //双差GNSS数据通过两个接收机和两个卫星之间的差分来消除某些误差（如卫星钟误差和接收机钟误差），从而提高定位精度。
            if (true)
            {
                /* during the DD observation, we only use the satellite with high elevation angle */
                //只使用高仰角卫星：卫星的仰角小于15度，则跳过该卫星的数据。
                ...
                //用 findMasterPrn 函数寻找主卫星。主卫星是用于双差计算的基准卫星。这里需要确保找到的主卫星是有效的（masterSatPrn >= 0）。
                int masterSatPrn = -1;
                ...
                //调用 gnssraw2SO 函数将原始GNSS数据转换为 单个观测数据singleObs 结构。
                ...
                /**********
                 * 准备双差测量数据：检查测站的GNSS数据中是否包含当前卫星的数据。
                 * 如果包含，则准备双差伪距和载波测量数据。
                 * 对于伪距测量，如果当前卫星不是主卫星，则标记为双差伪距测量（obs->DD_psr = true）。
                 * 如果载波相位数据有效，则标记为双差载波测量（obs->DD_car = true）。
                 * ***********/
                if(epochStGNSSMap.count(satPrn))
                {
    ...
    /******订阅完整的点云数据。revise
     * 清空 full_cloud 点云数据。
     * 将接收到的ROS消息中的点云数据转换为PCL点云格式。
     * 复制点云数据到一个新的点云对象 full 中。
     * 标记 new_full_cloud 为 true，表示已经接收到新的点云数据。
     * *******/
    void full_cloudHandler(const sensor_msgs::PointCloud2ConstPtr& pointCloudIn) {
        //ROS_INFO("full_cloudHandler star:%d",pointCloudIn->header);
        full_cloud->clear();
        ...
        new_full_cloud = true;
        //ROS_INFO("full_cloudHandler FINISH:%d",pointCloudIn->header);
     ...
     * 清空 surf_last 点云数据。
     * 将接收到的ROS消息中的点云数据转换为PCL点云格式。
     * 标记 new_surf 为 true，表示已经接收到新的表面点云数据。
     * *****/
    void surfaceLastHandler(const sensor_msgs::PointCloud2ConstPtr& pointCloudIn) {
        //ROS_INFO("surfaceLastHandler star:%d",pointCloudIn->header);
        surf_last->clear();
        pcl::fromROSMsg(*pointCloudIn, *surf_last);
        new_surf = true;
        //ROS_INFO("surfaceLastHandler FINISH:%d",pointCloudIn->header);
        //ROS_INFO("surfaceLastHandler num:%d",pointCloudIn->data.size());
    }

    /* subscribe the odometry from lidar odometry： 3Hz */
    /****订阅来自激光雷达里程计的位姿数据
     * 更新 time_new_odom 为当前接收到的里程计数据的时间戳。
     * 将当前的里程计数据保存到 odom_cur 中。
     * 标记 new_odom 为 true，表示已经接收到新的里程计数据
     * ****/
    void odomHandler(const nav_msgs::Odometry::ConstPtr& odomIn) {
        //ROS_INFO("odomHandler star:%d",odomIn->header);

        time_new_odom = odomIn->header.stamp.toSec();
        odom_cur = odomIn;

        new_odom = true;
        ROS_INFO("odomHandler FINISH:%d, %.3f, %.3f, %.3f",odomIn->header,odomIn->pose.pose.position.x,odomIn->pose.pose.position.y,odomIn->pose.pose.position.z);
    }

    /* subscribe the relative odometry (between two scan) from lidar odometry: 10Hz */
    /****
     * 功能：订阅来自激光雷达里程计的相对位姿数据（每秒10次）。
     * 将接收到的相对位姿数据添加到 each_odom_buf 缓冲区中。
     * 如果缓冲区的大小超过5000，则将最早的数据清空（以防止内存过多）。
     * 标记 new_each_odom 为 true，表示已经接收到新的相对位姿数据。
     * *****/
    void eachOdomHandler(const nav_msgs::Odometry::ConstPtr& odomIn) {
        //ROS_INFO("eachOdomHandler star:%d",odomIn->header);
        each_odom_buf.push_back(odomIn);
        //revise
        if(each_odom_buf.size() > 500)
            each_odom_buf[each_odom_buf.size() - 501] = nullptr;

        new_each_odom = true;
        //ROS_INFO("eachOdomHandler FINISH:%d",odomIn->header);
        ROS_INFO("eachOdomHandler  eachOdomHandler FINISH:%d, %.3f, %.3f, %.3f",odomIn->header,odomIn->pose.pose.position.x,odomIn->pose.pose.position.y,odomIn->pose.pose.position.z);
    }
    ...
    void imuHandler(const sensor_msgs::ImuConstPtr& ImuIn) {
        ...
        //首次接收到IMU数据，
        if (!first_imu) {
            //将IMU消息中的四元数转换为旋转矩阵，并存储在 Rs[0] 中。
            ...
            first_imu = true;
            //提取IMU消息中的线加速度和角速度，并存储在 acc_0 和 gyr_0 中。
            ...
            //创建一个新的预积分对象 Preintegration，并将其添加到 pre_integrations 向量中。设置预积分对象的重力向量为 -g。
    ...
    //将一个输入点 pi 变换到输出点 po。变换基于当前存储在 abs_pose 中的位姿（四元数和位移）。
    ...
    /* transform a point based on the given position */
    //将一个输入点 pi 变换到输出点 po。变换基于当前存储在 abs_pose 中的位姿（四元数和位移）。
    //使用作为参数传递的四元数 quaternion 和位移 transition。
    ...
    /* transform point clouds */
    //将一个点云 cloudIn 转换到另一个坐标系中，使用全局位姿 abs_pose
    ...
    /* transform point clouds */
    //将一个点云 cloudIn 转换到另一个坐标系中，使用给定的位姿 PointPoseInfo
    ...
    /* process imu data */
    //处理IMU数据，并根据IMU数据更新位姿信息。
    /*****
     * 检查 pre_integrations 是否小于 abs_poses，如果是，则创建新的预积分对象并添加到 pre_integrations 中，同时更新其他相关向量。
     * 计算未经修正的加速度和角速度。
     * 更新旋转矩阵 Rs。
     * 计算平均加速度并更新位置 Ps 和速度 Vs。
     * 将时间间隔、线性加速度和角速度推入预积分对象中。
     * 更新当前的加速度和角速度
     * *******/
    ...
    /*******
     * 这个函数根据目标时间 targetT 找到时间最近的两个索引 lower_idx 和 upper_idx。详细步骤如下：
     * 初始化差值 diff 为一个大数值。
     * 遍历 keyframe_idx，找到时间小于 targetT 且差值最小的索引 lower_idx。
     * 再次初始化差值 diff 为一个大数值。
     * 再次遍历 keyframe_idx，找到时间大于 targetT 且差值最小的索引 upper_idx。
     * *******/
    ...
    //getGlobalLowerUpperIdx 函数用于找到全局关键帧时间序列中最接近目标时间 targetT 的两个索引 lower_idx 和 upper_idx。
    ...
    //getGNSSDDMeasurementsWithinPeriod 函数用于在给定时间段内获取GNSS双差（Double-Differencing）测量值。
    /*******
     * 初始化 gnss_raw_map 的迭代器 iter_pr。
     * 遍历 gnss_raw_map，对于每个GPS时间索引 gps_time_idx：
     * 计算实际的GPS时间 iter_gps_time 。
     * 检查 iter_gps_time 是否在 windowStartTime 和 windowEndTime 之间。
     * 如果在时间窗口内，找到与当前时间索引最接近的参考站GNSS数据 st_gnss_data。
     * 计算参考站和当前时间索引的差值 diff，找到最小差值对应的索引 ref_idx。
     * 如果最小差值 max_diff 小于 300，说明找到的参考站数据有效，将当前用户GNSS测量数据和参考站GNSS测量数据分别推入 user_gnss_meas_vec_tmp 和 ref_gnss_meas_vec_tmp。
     * *********/
    ...
    //从用户和参考站的GNSS数据中提取并准备双差伪距（DD Pseudo-Range, DD Psr）观测数据。
    /********
     * 数据过滤和配对：函数首先检查并过滤来自用户和参考站的GNSS数据，只保留伪距大于1000且匹配的卫星数据。
     * 主卫星选择：在过滤后的数据中选择一个具有最高高度角的卫星作为主卫星（Master Satellite）。
     * 卫星系统检测：每个函数特定处理某个卫星导航系统（GPS、BDS、GAL、GLO），通过调用m_GNSS_Tools中的相应函数来检测卫星系统类型。
     * *********/
    ...
    //这个函数用于从给定的GNSS数据中找到主卫星（即高度角最大的卫星）。
    /****
     *  输入参数：
        gnss_data：包含多个GNSS观测数据的数组。
        mIndex：用于返回主卫星在数组中的索引。
        输出：
        返回主卫星的PRN（伪随机噪声码）。
     * 
     * *****/
    int getMasterPrn(nlosExclusion::GNSS_Raw_Array gnss_data, int &mIndex)
    {
        ...
        for(int m = 0;  m < gnss_data.GNSS_Raws.size(); m++) //
        {
            ...
            // 如果当前卫星的高度角大于当前最大值，则更新最大高度角和主卫星PRN
            if(fabs(ele)>maxEle)
            {
                maxEle = ele;
    ...
    //这个函数用于构建D矩阵。D矩阵用于双差观测，将GNSS伪距观测转换为双差伪距观测。
    /****
     * 输入参数：
     * gnss_data：包含多个GNSS观测数据的数组。
     * mPrn：主卫星的PRN。
    ...
    //该函数用于添加GNSS双差伪距残差块到Ceres优化问题中。
    /******
     * 输入参数：
    problem：Ceres优化问题实例。
    user_gnss_data：用户GNSS数据。
    ref_gnss_data：参考站GNSS数据。
    mPrn：主卫星的PRN。
    weight_matrix：权重矩阵。
    leftKey、rightKey：左右关键帧的索引。
    ts_ratio：时间比例。
    DDpsrThreshold：双差伪距阈值。
    输出：
    返回布尔值，表示操作是否成功。
     * 
     * 
    ...
    //该函数用于全局添加GNSS双差伪距残差块到Ceres优化问题中。
    ...
    //该函数用于添加GNSS因子到优化问题中，包括处理关键帧和GNSS测量数据。
    //处理关键帧和GNSS测量数据，并添加GNSS因子到优化问题中。
    /******
     * 该函数处理GNSS数据，并在合适的情况下将GNSS因子添加到优化问题中。
     * 包含了对系统初始化的检查、关键帧的处理、GNSS测量数据的过滤和处理、以及将GPS因子添加到局部位姿图中
     * *******/
    void addGNSSFactor()
    {
    ...
    //addLIOFactor 函数用于在局部位姿图（local pose graph）中添加激光惯性里程计（LIO）的因子，这些因子包括先验因子和相邻关键帧之间的相对位姿因子。
    //这个函数是关键帧滑动窗口内进行图优化的关键部分。
    void addLIOFactor() {
        // 获取关键帧数量和当前处理关键帧的索引：
        int keyframe_size = pose_keyframe->points.size();
        int proc_kf_idx = keyframe_size - slide_window_width;
        //add poses to global graph
        // 当关键帧数量达到滑动窗口大小时，初始化全局位姿图
        if (keyframe_size == slide_window_width) {
            ...
            // 当关键帧数量超过滑动窗口大小时，添加相邻关键帧之间的相对位姿因子
    ...
    /****
     * optimizeSlidingWindowWithLandMark 函数通过滑动窗口优化技术，结合 LiDAR、IMU 和 GNSS 数据，提升了系统的定位精度。
     * 它通过对多个因子的加权优化，逐步优化每个关键帧的位姿和速度，最终实现更高精度的位姿估计。
     * ****/
...
//        Timer t_optimize_slidingwindow("optimizeSlidingWindowWithLandMark");
        ...
        //ROS_WARN("surf_local_map_ds->points.size()= %d",surf_local_map_ds->points.size());
        ...
        for (int iterCount = 0; iterCount < iteration_num; ++iterCount) {
            ...
            ROS_INFO("Adding parameter blocks for optimization");
            for (int i = keyframe_idx[keyframe_idx.size()-slide_window_width]; i <= keyframe_idx.back(); i++){

                if (iterCount == 0) {
                    ROS_INFO("Processing index %d", i);
                    Eigen::Quaterniond tmpQ(Rs[i]);
                    ROS_ERROR("Quaternion %.3f, %.3f,%.3f,%.3f,", tmpQ.w(),tmpQ.x(),tmpQ.y(),tmpQ.z());
                    if (std::isnan(tmpQ.w()) || std::isnan(tmpQ.x()) || std::isnan(tmpQ.y()) || std::isnan(tmpQ.z()))
                    {
                        ROS_ERROR("Quaternion contains NaN values at index %d", i);
                        continue;
                    }
                ...
                //ROS_INFO("Quaternion added: w=%f, x=%f, y=%f, z=%f", tmpQ.w(), tmpQ.x(), tmpQ.y(), tmpQ.z());
            ...
            */
            ...
            for (int idx = keyframe_idx[keyframe_idx.size()-slide_window_width]; idx <= keyframe_idx.back(); idx++) {
                ...
                ROS_INFO ("Eigen::Quaterniond q: %.3f,%.3f,%.3f,%.3f",Q2.x(),Q2.y(),Q2.z(),Q2.w());
                ROS_INFO ("Eigen::Vector3d t: %.3f,%.3f,%.3f,%.3f",T2.x(),T2.y(),T2.z());

                ROS_INFO ("Eigen::Quaterniond Q2_: %.3f,%.3f,%.3f,%.3f",Q2_.x(),Q2_.y(),Q2_.z(),Q2_.w());
                ROS_INFO ("Eigen::Vector3d T2_: %.3f,%.3f,%.3f,%.3f",T2_.x(),T2_.y(),T2_.z());
                ...
                if (surf_local_map_ds->points.size() > 50) {
                    ...
                    //ROS_WARN("finish findCorrespondingSurfFeatures");
                    featureSelection(idx-1, Q2_, T2_);
                    //ROS_WARN("finish featureSelection");
                ...
                //ROS_WARN("finish keyframe_idx%d",idx);

            }
             ROS_WARN("finish the lidar factor");
#endif
...
//                ROS_WARN("bad bg3!!!!!!!!!!");
        ...
        ROS_WARN("finish the optimise");

//        t_optimize_slidingwindow.tic_toc();
    }

    /* 3D LiDAR Aided GNSS batch optimization */
    //optimizeBatchWithLandMark 函数通过融合LiDAR和GNSS数据，并使用Ceres求解器对批处理关键帧进行优化，提高了位姿估计的准确性。
    //函数包括初始化、状态设置、约束添加、优化运行和结果发布等步骤。
    void optimizeBatchWithLandMark() {
        //1.初步检查和设置:
        //函数首先检查是否至少有30个关键帧可用。如果不足，则提前返回。
        //如果启用了批量融合，则确保自上次批量激活以来至少增加了10个新关键帧，然后才继续.
        if (pose_info_keyframe->points.size() < 30) return;

        ROS_WARN("optimizeBatchWithLandMark start");
...
//        Timer t_optimize_batch("optimizeBatchWithLandMark");
        ...
        *pose_info_keyframe_batch = *pose_info_keyframe; //pose of each frame
        ...
        //3.生成全局地图用于LiDAR匹配:重置全局和下采样的表面地图用于LiDAR数据。
        surf_global_map.reset(new pcl::PointCloud<PointType>());
        surf_global_map_ds.reset(new pcl::PointCloud<PointType>());

        //multiple iterations enables the re-searching of correspondence
        //4.批量优化迭代:
        //设置优化迭代次数和对应的双差分伪距（DDpsr）阈值。
        //为每次迭代初始化Ceres问题。
        int iteration_num = 4;
        ...
        for (int iterCount = 0; iterCount < iteration_num; iterCount++) { // 1
            ...
            //5.GNSS测量:
            //获取指定时间段内的GNSS测量。
            //初始化接收器时钟漂移的数组，并将其添加为Ceres问题的参数块。
            std::vector<nlosExclusion::GNSS_Raw_Array> user_gnss_meas_vec_tmp, ref_gnss_meas_vec_tmp;
            if(gnss_meas_vec.size()>4)
            {
            ...
            //eigen to double: initialize the states
            //6.初始化状态:
            //将Eigen向量转换为双精度数组，并初始化批处理中的每个关键帧的状态。
            //将平移、四元数方向、IMU速度和偏差添加为Ceres问题的参数块。
...
#if 1 //pub batch trajectory 发布批处理路径和保存结果:
...
//        t_optimize_batch.tic_toc();
        ...
        ROS_WARN("optimizeBatchWithLandMark over");
...
//            vector<Eigen::Matrix<double, 6, 1>, Eigen::aligned_allocator<Eigen::Matrix<double, 6, 1>>> tmp_pnc;
    ...
    /* update the pose */
    //updatePose 函数用于更新关键帧的位姿信息。它会遍历滑动窗口内的关键帧，并将绝对位姿信息更新到关键帧的位姿数组中
    void updatePose() {
        abs_pose = abs_poses.back(); //获取最后一个绝对位姿，赋值给 abs_pose。
    ...
    //optimizeLocalGraph 函数用于优化局部图的位姿。它通过Ceres求解器优化滑动窗口内的关键帧位姿.
    /*******
     * 1.初始化四元数参数化和Ceres问题。
     * 2.计算当前滑动窗口内关键帧的数量 numPara，如果数量为0，则返回。
     * 3.初始化双精度数组 dQuat 和 dTrans，存储每个关键帧的平移和四元数信息。
     * 4.遍历滑动窗口内的所有关键帧，并将它们的平移和四元数信息添加到Ceres问题的参数块中。
     * 5.为每个关键帧添加LiDAR姿态因子到Ceres问题中。
     * 6.设置Ceres求解器选项，并求解优化问题。
     * 7.将优化后的结果更新到每个关键帧的位姿数组中。
     * ******/
    ...
    //该函数用于构建局部地图，通过最近的关键帧信息更新局部地图，结合 LiDAR 和 GNSS 数据提高精度。
    /*******
     * 1.初始化阶段：
     * 检查是否有关键帧。如果没有，使用初始的姿态信息（q_bl 和 t_bl）变换最后一次的 LiDAR 点云（surf_last），并将其添加到局部地图（surf_local_map）。
     * 2.检查并更新最近的表面关键帧：
     * 如果最近的表面关键帧数小于局部地图的宽度（local_map_width），则清空最近的表面关键帧，并从最新的关键帧向后遍历，收集关键帧，直到收集到足够的关键帧或遍历完成。
     * 如果最近的表面关键帧数已经达到或超过局部地图的宽度，则检查最新的帧索引是否更新。如果更新，则从最近的表面关键帧中移除最早的一个，并添加最新的关键帧。
     * 3.更新局部地图：
     * 清空局部地图的点云。
     * 遍历最近的表面关键帧，将每个关键帧的点云添加到局部地图中。
     * 通过这个函数，可以在局部地图中保持最新的环境信息，提高后续定位和导航的准确性。
     * ********/
    void buildLocalMapWithLandMark() {
        ...
        //ROS_ERROR("Initialization finished");
        // 检查是否已经有足够的关键帧用于局部地图构建
        if (recent_surf_keyframes.size() < local_map_width) {
            recent_surf_keyframes.clear();

            // 从最新的关键帧向后遍历，直到收集到足够的关键帧或遍历完成
    ...
    //该函数用于对点云数据进行下采样，以减少点云的数量，提高处理效率。具体处理了局部地图、全局点云和最近一次扫描的下采样
    void downSampleCloud() {
        if (!surf_local_map || !surf_local_map_ds || !full_cloud || !surf_last || !surf_last_ds)
        {
            ROS_ERROR("Null pointer detected in downSampleCloud");
            return;
        }

        ds_filter_surf_map.setInputCloud(surf_local_map);
        //ROS_INFO("full_cloud %d",surf_local_map->points.size());
        //ROS_INFO("ds_filter_surf_map.filter start");
        ds_filter_surf_map.filter(*surf_local_map_ds);
        //ROS_INFO("ds_filter_surf_map.setInputCloud start");
        pcl::PointCloud<PointType>::Ptr fullDS(new pcl::PointCloud<PointType>());
        //ROS_INFO("ds_filter_surf_map.setInputCloud start");
        ds_filter_surf_map.setInputCloud(full_cloud);
        //ROS_INFO("full_cloud %d",full_cloud->points.size());
        ds_filter_surf_map.filter(*fullDS);
        full_clouds_ds.push_back(fullDS);
        //ROS_INFO("ds_filter_surf_map.clear start");
        surf_last_ds->clear();
        ds_filter_surf.setInputCloud(surf_last);
        ds_filter_surf.filter(*surf_last_ds);
        //ROS_INFO("ds_filter_surf_map.filter finsh");
        return;
    }

    //findCorrespondingSurfFeatures 函数用于查找给定索引 idx 的表面特征点的对应特征点。
    //该函数通过将点云点变换到地图坐标系中，然后使用最近邻搜索找到对应的特征点，并通过平面拟合和权重计算来判断平面是否有效。
...
//        Timer t_feature_association("findCorrespondingSurfFeatures");
        ...
        //ROS_INFO("KD tree input cloud size: %zu", surf_local_map_ds->points.size());

        if (surf_local_map_ds->points.empty())
        {
        ...
        for (int i = 0; i < surf_frames[idx]->points.size(); ++i) {
            ...
            //ROS_ERROR("NearestKSearch Nearest neighbor search afinshed %d", surf_frames[idx]->points.size());
            ...
            // 检查 pt_in_map 是否包含 NaN 或无效值
            if (std::isnan(pt_in_map.x) || std::isnan(pt_in_map.y) || std::isnan(pt_in_map.z))
            {
                ROS_ERROR("pt_in_map contains NaN values at index %d: (%f, %f, %f)", i, pt_in_map.x, pt_in_map.y, pt_in_map.z);
                continue;
            }
            ...
            if (kd_tree_surf_local_map->nearestKSearch(pt_in_map, 5, pt_search_idx, pt_search_sq_dists) <= 0)
            {
                ROS_ERROR("NearestKSearch failed at index %d", i);
                continue;
            }

            //ROS_INFO("NearestKSearch success at index %d: pt_search_idx[0] = %d, pt_search_sq_dists[0] = %f", i, pt_search_idx[4], pt_search_sq_dists[4]);
            //ROS_ERROR("NearestKSearch Nearest neighbor search at index %d %.8f",i, pt_search_sq_dists);
            //ROS_ERROR("NearestKSearch Nearest neighbor search afinshed %d", i);
...
//        t_feature_association.tic_toc();
        ...
        //ROS_INFO("KD tree        successcloud size: %d", count_);
...
//     }
    ...
    //findGlobalCorrespondingSurfFeatures_Batch 函数用于批量查找全局对应的表面特征点。
    //该函数通过将点云点变换到全局坐标系中，然后使用最近邻搜索找到对应的特征点，并通过平面拟合和权重计算来判断平面是否有效。
...
//        t_feature_association.tic_toc();
    ...
    //findGlobalCorrespondingSurfFeaturesAdd_Batch 函数与 findGlobalCorrespondingSurfFeatures_Batch 类似，主要区别在于它用于批量添加全局对应的表面特征点。
...
//        t_feature_association.tic_toc();
    ...
    //featureSelection 函数用于从当前帧的特征点中选择一个子集，以便于后续的优化和处理。该函数通过随机选择的方式从特征点中挑选出符合条件的特征点。
    void featureSelection (int idx, Eigen::Quaterniond q, Eigen::Vector3d t) {
//        Timer t_feature_select("FeatureSelector");

        ROS_WARN("featureSelection star");

        int idVec = idx - keyframe_idx[keyframe_idx.size() - slide_window_width] + 1;

        if (idVec < 0 || idVec >= vec_surf_cur_pts.size())
        {
            ROS_ERROR("idVec out of range: %d", idVec);
            return;
        }

        if (!vec_surf_cur_pts[idVec] || !vec_surf_normal[idVec])
        {
            ROS_ERROR("Null pointer detected in featureSelection");
            return;
        }
        ...
        while (surf_cur_pt_temp->points.size() < feature_res_num && random_select) {
            ...
            //std::vector<int> rand_ids = rgi_.geneRandArrayNoRepeat(0, vec_surf_cur_pts[idVec]->points.size() - 1, rand_set_num);
            ...
            if (selected_id < 0 || selected_id >= vec_surf_cur_pts[idVec]->points.size())
            {
                ROS_ERROR("selected_id out of range: %d", selected_id);
                continue;
            }
...
//        t_feature_select.tic_toc();
    ...
    //globalFeatureSelection_Batch 函数用于在全局范围内批量选择特征点。该函数通过随机选择的方式从特征点中挑选出符合条件的特征点，并更新特征点的集合
...
//        t_feature_select.tic_toc();
    ...
    //globalFeatureSelectionAdd_Batch 函数与 globalFeatureSelection_Batch 类似，主要区别在于它用于批量添加全局对应的特征点。
...
//        t_feature_select.tic_toc();
    ...
    //toEulerAngle 函数将四元数转换为欧拉角（roll, pitch, yaw）。
    //四元数提供了一种表示旋转的方式，而欧拉角则是另一种常用的表示方法。这些旋转角度围绕固定的 x, y, z 轴进行。
    ...
    //计算 GNSS 和 IMU 之间的平移向量。这个平移向量是通过将四元数转换为旋转矩阵，然后与 GNSS 和 IMU 之间的变换矩阵相乘得到的。
    ...
    //saveKeyFramesAndFactors 函数负责保存关键帧和相关因子，并进行局部优化和全局图优化。
    //该函数在SLAM系统中通过保存关键帧、处理IMU数据、优化滑动窗口、添加因子等步骤实现位姿优化。
    /*******
     * 函数解释:
     * 保存关键帧数据：将当前位姿、关键帧ID和时间戳等信息保存到对应的数据结构中。
     * 处理IMU数据：根据IMU数据更新位姿。
     * 优化滑动窗口：根据关键帧数量和IMU数据进行滑动窗口优化。
     * 局部图优化：添加局部因子图并进行优化，结合GNSS数据进行更新。
     * 全局图优化：添加关键帧之间的因子到全局图，并进行全局优化。
     * *******/
...
//        Timer t_sFF("saveKeyFramesAndFactors");
        ...
        *surfEachFrame = *surf_last_ds;
        ...
        while(imu_buf[i]->header.stamp.toSec() < timeodom_cur) {
            //ROS_INFO("Timestamp synchronized, please check your hardware!,%3.f",imu_buf[i]->header.stamp.toSec());
...
//        t_sFF.tic_toc();
    ...
    //correctPoses函数在检测到回环并进行全局优化后，确保所有姿态都得到一致的更新。
    //这包括重新计算相对姿态、更新全局姿态、并将这些更新集成回系统的姿态表示中。
    void correctPoses() {
        //1.回环检测检查
        if (loop_closed == true) {
            //2.清除最近的关键帧：
            ...
            //3.计算相对姿态：函数计算滑动窗口内连续关键帧之间的相对姿态（四元数和平移）。这些相对姿态存储在 quaternionRel 和 transitionRel 向量中。
            ...
            //4.更新全局姿态：函数根据全局优化的估计值更新所有帧的全局姿态。更新 pose_each_frame 和 pose_info_each_frame 中的点的位置信息和方向。
            ...
            //5.更新关键帧姿态：根据全局优化的姿态更新关键帧姿态。更新 pose_keyframe 和 pose_info_keyframe 中的关键帧姿态信息。
            //同时更新绝对姿态（ abs_poses ）及其对应的旋转矩阵（Rs）和位置（Ps）。
            ...
            //6.更新滑动窗口内的姿态：对滑动窗口内的姿态进行积分，基于相对姿态更新平移和旋转。
            for(int i = abs_poses.size() - slide_window_width; i < abs_poses.size() - 1; i++) {
            ...
            //7.最终姿态更新：更新最终姿态（abs_pose）为绝对姿态列表中的最后一个元素。同时更新最后一个关键帧对应的姿态（last_pose）和选择的姿态（select_pose）。
            abs_pose = abs_poses.back();
            for (int i = 0; i < 7; ++i) {
                last_pose[i] = abs_poses[abs_poses.size() - slide_window_width][i];
            }
    ...
    //publishOdometry函数主要负责在ROS系统中发布当前的里程计信息、导航卫星信息和点云数据，同时更新并发布相关的TF变换。
    //通过这些信息的发布，确保外部系统可以获取到最新的定位和姿态数据。
    void publishOdometry() {

        //1.检查是否满足滑动窗口宽度条件：如果 pose_info_keyframe 中的点数达到或超过滑动窗口的宽度，则继续执行，否则跳过。
        if(pose_info_keyframe->points.size() >= slide_window_width) {

            //2.设置当前时间戳和姿态信息：获取当前时间戳time_new_odom并设置里程计消息odom_mapping的时间戳和姿态信息。
            ...
            //3.设置速度信息：设置IMU的加速度偏差和角速度偏差为当前里程计消息的速度信息。
...
//            fout_evo.close();
            ...
            //5.发布导航卫星信息：将ENU坐标转换为ECEF坐标，再转换为LLA坐标，并发布GNSS导航卫星信息
            geometry_msgs::PoseStamped enu_pose_msg;
            enu_pose_msg.header = rtk_ini_enu_path.header;
        ...
        //9.发布点云信息：如果有订阅者，则发布滑动窗口中的点云信息，包括局部地图、全局地图和表面特征点。
    ...
    //publishLCOdometry函数主要负责发布和保存回环检测优化后的里程计数据。
    //它将优化后的位姿转换为适当的格式，并发布相关的ROS消息，同时将结果记录到文件中以便后续分析.
...
//            fout_evo.close();
    ...
    //clearCloud 函数主要用于清理旧的点云数据，以节省内存并保持系统性能。
    void clearCloud() {
        ROS_INFO("clearCloud start");
...
//        }
    ...
    //loopClosureThread 函数是一个循环检测线程，用于执行回环检测和闭环优化
    void loopClosureThread() {
        if (!loop_closure_on)
            return;
        ROS_INFO("loopClosureThread start,%d",loop_closure_on);
        ros::Rate rate(1);
        while (ros::ok()) {
            rate.sleep();
            ROS_INFO("performLoopClosure start");
            performLoopClosure();
            ROS_INFO("performLoopClosure over");
        }
        ROS_INFO("loopClosureThread over");
    }

    //detectLoopClosure 函数用于检测回环闭合点，并准备进行闭环优化所需的点云数据。
    bool detectLoopClosure() {
        ...
        //3.寻找最近的关键帧：
        //使用 kd_tree_his_key_poses->radiusSearch(select_pose, lc_search_radius, pt_search_idxLoop, pt_search_sq_distsLoop, 0); 查找在指定半径内的历史关键帧。
        ...
        closest_his_idx = -1;
        //遍历搜索结果，找到与当前时刻差异超过阈值 lc_time_thres 的最近历史关键帧，并记录其索引 closest_his_idx。
        ...
        //判断是否找到合适的回环闭合点：
        if (closest_his_idx == -1)  //closest_his_idx 为 -1，表示没有找到合适的历史关键帧，
            return false;
...
//        ROS_INFO("******************* Loop closure ready to detect! *******************");
        ...
        //5.组合最新的关键帧点云：遍历最新的关键帧，根据其位姿信息和当前位姿，将点云转换到世界坐标系并组合起来。
        ...
        //6.滤波最新关键帧点云：使用体素滤波对组合后的最新关键帧点云进行降采样。
        ...
        //7.组合历史关键帧点云：遍历最近的历史关键帧，根据其位姿信息和当前位姿，将点云转换到世界坐标系并组合起来。
    ...
    //这个函数的主要作用是在回环检测到合适的闭环点后，使用ICP算法进行点云对齐，并将对齐后的位姿约束添加到全局优化图中，通过gtsam进行全局优化，从而校正整个轨迹的位姿。
    void performLoopClosure() {
        ...
        //3.初始化ICP（Iterative Closest Point）算法：设置ICP算法的各种参数，包括最大对应距离、最大迭代次数、转换误差、欧几里得拟合误差和RANSAC迭代次数。
        ...
        //4.执行ICP对齐：将 latest_key_frames_ds 作为输入源，his_key_frames_ds 作为目标，通过ICP进行对齐，并将结果存储在 alignedCloud 中。
        ...
        //5.检查ICP对齐结果：如果ICP未收敛或拟合得分大于阈值 lc_icp_thres，则返回。
...
//        ROS_INFO("******************* Loop closure detected! *******************");
        ...
        //6.计算校正变换：从 ICP 结果中获取最终的变换矩阵，并提取出旋转和平移增量。根据增量更新最新关键帧的位姿。
        ...
        //7.构建gtsam优化图：使用更新后的最新关键帧位姿和最接近的历史关键帧位姿，构建gtsam的位姿约束。将约束添加到全局优化图中，并更新gtsam优化器。
        ...
        //8.计算全局位姿估计，并调用 correctPoses 函数更新位姿。
        loop_closed = true;
...
//        ROS_INFO("******************* Loop closure finished! *******************");
    ...
    /*********
     * 发布完整地图 publishCompleteMap：
     * 该函数首先检查关键帧点云的数量是否大于10，如果是，则遍历关键帧点云并进行相应的变换。
     * 使用降采样滤波器对全局地图进行降采样。
     * 将处理后的点云数据转换为ROS消息并发布。
     * **********/
    ...
    /******
     * 地图可视化线程 mapVisualizationThread：
     * 使用一个较低频率（0.038 Hz）循环检查是否发布地图。
     * 如果启用了保存PCD文件功能，则遍历关键帧点云并保存处理后的点云数据到PCD文件中。
     * ********/
    ...
    /********
     * 后端融合线程 backendFusionThread：
     * 该线程以较高频率（10 Hz）循环执行批处理融合优化函数 optimizeBatchWithLandMark，如果启用了批处理融合功能。
     * ********/
    ...
    void run() {
        if (new_surf && new_odom && new_each_odom && new_full_cloud) {
            ...
            //ROS_INFO("buildLocalMapWithLandMark start");
            buildLocalMapWithLandMark(); // build local map // 构建带地标的局部地图
            //ROS_INFO("downSampleCloud start");
            downSampleCloud(); // downsample all the clouds // 对点云进行降采样
            //ROS_INFO("saveKeyFramesAndFactors start");
            saveKeyFramesAndFactors(); // 3D LiDAR Aided GNSS-RTK fusion // 保存关键帧和因子，进行3D LiDAR辅助的GNSS-RTK融合
            //ROS_INFO("publishOdometry start");
            publishOdometry(); // publish the latest // 发布里程计信息
            //ROS_INFO("publishOdometry finish");
            clearCloud();  // 清理点云
//            t_map.tic_toc();
//            runtime += t_map.toc();
...
int main(int argc, char** argv) {
    ...
    //4.创建一个 Estimator 类的对象 Estimator_。
    Estimator Estimator_;

    /* loop closure detection thread */
    //5.回环检测线程：创建一个新的线程 threadLoopClosure，用于执行 Estimator_ 对象的 loopClosureThread 方法。
    ...
    //6.地图可视化线程：创建一个新的线程 threadMapVisualization，用于执行 Estimator_ 对象的 mapVisualizationThread 方法。
    std::thread threadMapVisualization(&Estimator::mapVisualizationThread, &Estimator_);

    /* backend fusion thread */
    //7.后端融合线程：创建一个新的线程 threadBackendFusion，用于执行 Estimator_ 对象的 backendFusionThread 方法。
